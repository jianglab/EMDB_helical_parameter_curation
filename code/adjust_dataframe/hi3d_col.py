...
df['HI3D_link'] = 'https://helical-indexing-hi3d.streamlit.app/?emd_id=emd-' + df['emdb_id'].str.extract(r'EMD-(\d+)', expand=False) \
    + '&rise=' + df['rise_validated (Å)'].astype(str) + '&twist=' + df['twist_validated (°)'].astype(str) + '&csym=' + df['csym_validated'].str[1:]\
        + '&rise2=' + df['rise_deposited (Å)'].astype(str) + '&twist2=' + df['twist_deposited (°)'].astype(str) + '&csym2=' + df['csym_deposited'].str[1:]
